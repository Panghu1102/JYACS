# JYACS Blessland - JustYuriAIChatSubmod 官方前端子模组
# 版本: 1.0.0
# 作者: Panghu1102
# 
# 兼容性说明:
# - 依赖 jyacs-api.rpy 提供的 store.jyacs、store.jyacs_log 等
# - 依赖 dev.rpy 提供的 store.key_replace、store.TalkSplitV2 等
# - 所有AI交互、文本处理、分句显示都通过store统一接口
#
# 接口说明:
# 1. 文本处理: store.key_replace(text) - 关键词替换
# 2. 分句处理: store.TalkSplitV2() - 创建分句器
# 3. 日志记录: store.jyacs_log(message, level) - 统一日志
# 4. AI交互: store.jyacs.jyacs.* - AI相关功能
#
# 使用示例:
# - 处理用户输入: store.process_user_message("你好[player]")
# - 分句显示AI回复: store.process_ai_reply_with_splitter(ai_reply)
# - 应用设置: store.jyacs_apply_setting()
# - 重置会话: store.reset_session()
#
# 注册到store的函数:
# - process_user_input, create_sentence_splitter, process_ai_reply_with_splitter
# - process_user_message, jyacs_apply_setting, jyacs_apply_advanced_setting
# - reset_session, output_chat_history, upload_chat_history
# - change_chatsession, reset_player_information, export_player_information
# - update_model_setting, change_loglevel, change_conloglevel, log_eventstat
...
# 默认持久化数据
default persistent.jyacs_player_additions = []
...
init -990 python:
    ...
    if not hasattr(store, 'persistent'):
        store.persistent = store.object()
    
    if not hasattr(store.persistent, 'jyacs_setting_dict'):
        store.persistent.jyacs_setting_dict = {
            "console_font": "SarasaMonoTC-SemiBold.ttf",
            "log_level": "INFO",
            "auto_connect": True,
            "auto_reconnect": True,
    ...
    if not hasattr(store.persistent, 'jyacs_player_additions'):
        store.persistent.jyacs_player_additions = []
...
init -985 python:
    ...
    mobile_min_timescamp = 0
    if hasattr(store, 'get_build_timescamp') and hasattr(store, 'mobile_min_timescamp'):
        if get_build_timescamp() < mobile_min_timescamp and renpy.android:
            renpy.notify(_("> 你当前的生成版本过旧, 可能影响正常运行, 请升级至最新生成版本"))
...
define jyacs_confont = "mod_assets/font/SarasaMonoTC-SemiBold.ttf"
...
init 10 python:
    ...
    import copy
    jdef_setting = copy.deepcopy(jyacs_default_dict)
    ...
    if hasattr(store, 'jyacs') and hasattr(store.jyacs, 'jyacs') and hasattr(store.jyacs.jyacs, 'JyacsProviderManager'):
        ...
        # 创建默认的提供商管理器
        JyacsProviderManager = type('obj', (object,), {
            'servers': [
    ...
    def jyacs_reset_setting():
        """重置JYACS设置到默认值"""
        persistent.jyacs_setting_dict = jdef_setting.copy()
        if hasattr(store, 'jyacs_log'):
    ...
    def _upload_persistent_dict():
        ...
        keys_to_clear = [
            ...
            '_mas_compliments_database', '_mas_fun_facts_database', '_mas_mood_database',
    ...
    # 使用 store.key_replace 进行文本处理
    def process_user_input(text):
        """处理用户输入，使用 store.key_replace 进行关键词替换"""
        if hasattr(store, 'key_replace'):
            return store.key_replace(text)
        return text
    
    # 使用 store.TalkSplitV2 进行分句处理
