...
class ConfigTests(JyacsTestCase):
    ...
    def test_config_structure(self):
        """测试配置文件结构"""
        try:
            with open("jyacs_config.json", "r", encoding="utf-8") as f:
                config = json.load(f)
...
class EmotionTests(JyacsTestCase):
    ...
    def test_emotion_selection(self):
        ...
        for case in test_cases:
            with self.subTest(input=case["input"]):
                try:
                    emotion = self.selector.select_emotion(case["input"])
                    self.assertIsNotNone(emotion)
                    self.assertIn(emotion, self.selector.emotion_patterns[case["expected_type"]]["emotes"])
                except Exception as e:
