...
init -1400 python:
    ...
    try:
        ...
        class JyacsEmoSelector(object):
            def __init__(self): self.emote = "1eua"
            def get_emote(self): return self.emote
            def set_emotion(self, emote_code): self.emote = emote_code
            def analyze(self, text): return text
        print("JYACS: Could not import JyacsEmoSelector, using a dummy class.")
    ...
    class JyacsAi:
        ...
        def __init__(self):
            ...
            # 使用 JyacsEmoSelector 初始化 MoodStatus
        ...
        def init_connect(self):
            ...
            try:
                # 从持久化数据加载API设置
                self.api_key = persistent.jy_ai_api_key
                self.api_url = persistent.jy_ai_api_url
                self.model_name = persistent.jy_ai_model_name

                if not self.api_key or not self.api_url:
                    renpy.notify("API密钥和地址不能为空")
                    raise Exception("API密钥和地址不能为空")
...
label jyacs_do_connect:
    python:
        success = store.jyacs.init_connect()
    if success:
        jump submod_jyacs_talking
    else:
        show screen jyacs_settings
    return