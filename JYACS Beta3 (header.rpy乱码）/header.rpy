# JYACS Blessland - JustYuriAIChatSubmod 官方前端子模�?# 版本: 1.0.0
# 作�? Panghu1102

# 基础配置和路径设�?init -999 python:
    import store
    import os
    import sys
    import logging
...
# 持久化数据初始化
init -950 python:
    # 初始化默认设�?    if not hasattr(persistent, 'jyacs_setting_dict'):
...
init -800 python:
    if hasattr(store, 'jyacs') and hasattr(store.jyacs, 'jyacs') and hasattr(store.jyacs.jyacs, 'JyacsProviderManager'):
        ...
        # 创建默认的提供商管理�?        JyacsProviderManager = type('obj', (object,), {
            'servers': [
                {'id': 1, 'name': 'JYACS官方服务�?, 'deviceName': 'JYACS Official', 'servingModel': 'JYACS-Main', 'isOfficial': True},
                {'id': 2, 'name': '备用服务�?, 'deviceName': 'Backup Server', 'servingModel': 'JYACS-Core', 'isOfficial': False}
            ],
            'get_server_by_id': lambda server_id: {'name': 'Unknown', 'deviceName': 'Unknown', 'servingModel': 'Unknown'},
            'get_provider': lambda: []
        })
    ...
    def _upload_persistent_dict():
        ...
        # 清理MAS特有的数�?- 保留清理逻辑但简�?        keys_to_clear = [
        ...
        # 添加玩家信息 - 简化处�?        if hasattr(store, 'player'):
            d['playername'] = store.player
        
        # 过滤数据 - 简化处�?        keys_to_remove = []
        for i in d.keys():
            try:
                json.dumps(d[i])
    ...
    def process_user_message(message):
        """处理用户消息，包括关键词替换和日志记�?""
        # 使用 store.key_replace 进行关键词替�?        processed_message = process_user_input(message)
    ...
    def jyacs_apply_setting(ininit=False):
        ...
        if not ininit:
            ...
            renpy.notify(_("JYACS: 已上传设�?) if success else _("JYACS: 请等待连接就绪后手动上传"))
    ...
    def change_loglevel():
        ...
        # 安全地检查对象是否存�?        if hasattr(store, 'jyacs_submod_utils') and hasattr(store.jyacs_submod_utils, 'submod_log'):
            store.jyacs_submod_utils.submod_log.level = persistent.jyacs_setting_dict["log_level"]
    ...
    def try_eval(str):
        ...
        try:
            ...
            if hasattr(store, 'jyacs_log'):
                store.jyacs_log("Failed to eval: {}|param: '{}'".format(e, str), "ERROR")
            return None
    ...
    def log_eventstat():
        ...
        try:
            ...
            # 安全地记录事件状�?            if hasattr(store, 'jyacs_log'):
                # 使用安全的条件检查，避免不存在的标签
                try:
                ...
                # 对其他标签也做类似处�?                for label in ['jyacs_chr2', 'jyacs_chr_gone', 'jyacs_chr_corrupted2', 
...
# 子模组注册和基础函数
init -900 python:
    # 安全的日志记录函�?    def jyacs_log(message, level="INFO"):
        """简化的日志记录函数"""
        try:
    ...
    # 进度条函�?    def jyacs_progress_bar(percentage, current=None, total=None, bar_length=20):
        """进度条显示函�?""
...
# 子模组注�?init -900 python:
    # 子模组信�?- 使用安全的调用方�?    if hasattr(store, 'jyacs_submod_utils') and hasattr(store.jyacs_submod_utils, 'Submod'):
...
# 核心功能初始�?init -800 python:
    ...
    def jyacs_init():
        ...
        try:
            ...
            # 安全地设置日志级�?            if hasattr(store, 'jyacs_submod_utils') and hasattr(store.jyacs_submod_utils, 'submod_log'):
                ...
                # 安全地运行函�?                if hasattr(store.jyacs_submod_utils, 'getAndRunFunctions'):
                    store.jyacs_submod_utils.getAndRunFunctions()
            ...
            if hasattr(store, 'jyacs_log'):
                store.jyacs_log("JYACS初始化失�? {}".format(e), "ERROR")
            else:
                print("[JYACS-ERROR] JYACS初始化失�? {}".format(e))
            return False
    ...
    def get_build_timescamp():
        """获取构建时间�?""
        return 0  # 默认返回0，避免错�?
# 默认设置和配�?init 10 python:
    # 检查jyacs模块是否可用
    if not hasattr(store, 'jyacs') or not hasattr(store.jyacs, 'jyacs'):
        # 如果jyacs模块不可用，使用默认�?        jyacs_default_dict = {
            ...
            "42seed": False,
            "use_anim_background": False  # 移除MAS特有的动态背�?        }
    else:
        # 使用jyacs模块的�?        jyacs_default_dict = {
            "auto_reconnect": False,
    ...
    # 安全地更新设�?    if hasattr(persistent, 'jyacs_setting_dict'):
    jyacs_default_dict.update(persistent.jyacs_setting_dict)
    if hasattr(persistent, 'jyacs_advanced_setting'):
    jyacs_advanced_setting.update(persistent.jyacs_advanced_setting)

    # 更新持久化数�?    persistent.jyacs_setting_dict = jyacs_default_dict.copy()
    persistent.jyacs_advanced_setting = jyacs_advanced_setting.copy()

# 工具函数
init -1800 python:
    def process_user_input(text):
        """处理用户输入，使�?store.key_replace 进行关键词替�?""
        if hasattr(store, 'key_replace'):
            return store.key_replace(text)
        return text
    ...
    def process_ai_reply_with_splitter(reply_text):
        """使用 store.TalkSplitV2 处理AI回复的分句显�?""
    ...
    # 移除MAS特定的装饰器，改为普通函�?    def _upload_persistent_dict():
        """上传持久化数据到JYACS"""
        if not hasattr(store, 'jyacs') or not hasattr(store.jyacs, 'jyacs'):
            if hasattr(store, 'jyacs_log'):
                store.jyacs_log("JYACS模块不可用，跳过数据上传", "WARNING")
...
screen jyacs_setting_pane():
    python:
        ...
        stat = _("未连�?) if not jyacs.jyacs.wss_session else _("已连�?) if jyacs.jyacs.is_connected() else _("已断开")
        # 安全地获取API密钥
    ...
    vbox:
        ...
        # 定义安全的版本检查函�?        python:
            def safe_version_check():
                # 默认返回False，避免显示不必要的警�?                return False
                
        # 使用安全的版本检�?        if safe_version_check():
            text _("> 你当前的版本过旧, 可能影响正常运行, 请升级至最新版�?):
                xalign 1.0 yalign 0.0
                xoffset -10
                style "main_menu_version"
                
        if store.jyacs.jyacs.is_outdated is None:
            text _("> 无法验证版本�? 如果出现问题请更新至最新版"):
                xalign 1.0 yalign 0.0
                xoffset -10
                style "main_menu_version"
        elif store.jyacs.jyacs.is_outdated is True:
            text _("> 当前版本已不再支�? 请更新至最新版"):
                xalign 1.0 yalign 0.0
                xoffset -10
                style "main_menu_version"
        ...
        if hasattr(store, 'jyacs_submod_utils') and hasattr(store.jyacs_submod_utils, 'isSubmodInstalled'):
            if store.jyacs_submod_utils.isSubmodInstalled("Better Loading"):
                text _("> 警告: �?Better Loading 不兼�?):
                    xalign 1.0 yalign 0.0
                    xoffset -10
                    style "main_menu_version"
            if store.jyacs_submod_utils.isSubmodInstalled("Log Screen"):
                text _("> 警告: �?Log Screen 一起使用时, 请将'submod_log'的详细程度提高至info及以�?):
                    xalign 1.0 yalign 0.0
                    xoffset -10
                    style "main_menu_version"
                    
        text _("> JYACS通信状�? [jyacs.jyacs.status]|[jyacs.jyacs.JyacsAiStatus.get_description(jyacs.jyacs.status)]"):
            xalign 1.0 yalign 0.0
            xoffset -10
            style "main_menu_version"
        ...
        if jyacs.jyacs.has_token() and not jyacs.jyacs.is_connected():
            textbutton _("> 使用已保存令牌连�?):
                action Function(store.jyacs.jyacs.init_connect)
            ...
            textbutton _("> 上传对话历史到会�?'[store.jyacs.jyacs.chat_session]'"):
                action Function(upload_chat_history)

            textbutton renpy.substitute(_("> 退出当前DCC账号")) + " " + renpy.substitute(_("{size=-10}* 如果对话卡住�? 点我断开连接")):
        ...
        vbox:
            ...
            viewport:
                ...
                vbox:
                    ...
                    hbox:
                        # 修改�?太空教室"的引用，使其更通用
                        textbutton _("{size=15}因能力有�? 阅读信件后信件列表将在返回主界面后重新显�?")
                    ...
                    for postal in persistent._jyacs_send_or_received_mpostals:
                        ...
                        text renpy.substitute(_("信件状�? ")) + postal["responsed_status"]:
                            size 10                       
                        ...
                        hbox:
                            ...
                            if postal["responsed_content"] != "":
                                # 使用安全的角色名变量
                                textbutton _("阅读[character_name]的回�?):
                                    action [
                                            Hide("jyacs_mpostals"),
                                            Hide("jyacs_setting"),
                                            Function(store.jyacs_apply_setting),
        ...
        vbox:
            ...
            viewport:
                ...
                vbox:
                    ...
                    hbox:
                        text _("{size=-10}注意: 只有已被勾�?标记了X)的高级设置才会被使用, 未使用的设置将使用服务端的默认设�?)
                    hbox:
                    ...
                    hbox:
                        textbutton "top_p":
                            ...
                            hovered SetField(_tooltip, "value", _("模型选择的范�? 模型考虑概率质量值在�?top_p 的标记的结果, 因此�?.1 意味着仅考虑概率质量值前 10% 的标�?))
                    ...
                    hbox:
                        textbutton "temperature":
                            ...
                            hovered SetField(_tooltip, "value", _("模型输出的随机�? 较高的值会使输出更随机, 而较低的值则会使其更加专注和确定"))
                            unhovered SetField(_tooltip, "value", _tooltip.default)
                    ...
                    hbox:
                        textbutton "max_tokens":
                            ...
                            hovered SetField(_tooltip, "value", _("模型输出的长度限�? 较高的值会使输出更�?))
                            unhovered SetField(_tooltip, "value", _tooltip.default)
                    ...
                    hbox:
                        textbutton "frequency_penalty":
                            ...
                            hovered SetField(_tooltip, "value", _("频率惩罚, 正值基于新标记在文本中的现有频率对其进行惩�? 降低模型重复相同行的可能�?))
                            unhovered SetField(_tooltip, "value", _tooltip.default)
                    ...
                    hbox:
                        textbutton "presence_penalty":
                            ...
                            hovered SetField(_tooltip, "value", _("重现惩罚, 正值基于新标记出现在文本中的情况对其进行惩�? 增加模型谈论新话题的可能�?))
                            unhovered SetField(_tooltip, "value", _tooltip.default)
                    ...
                    hbox:
                        textbutton "tnd_aggressive":
                            ...
                            hovered SetField(_tooltip, "value", _("当其�?时只调用MFocus直接选择的工�? �?时总是会调用时间与节日工具. �?时还会额外调用日期工�?\n当其�?且mas_geolocation存在�? tnd_aggressive还会额外调用当前天气工具.\n越高越可能补偿MFocus命中率低下的问题, 但也越可能会干扰模型对部分问题的判断."))
                            unhovered SetField(_tooltip, "value", _tooltip.default)
                    ...
                    hbox:
                        textbutton "mf_aggressive:[persistent.jyacs_advanced_setting.get('mf_aggressive', 'None')]":
                            ...
                            hovered SetField(_tooltip, "value", _("总是尽可能使用MFocus的最终输出替代指导构型信�?\n启用可能提升模型的复杂信息梳理能�? 但也可能会造成速度下降或专注扰�?))
                            unhovered SetField(_tooltip, "value", _tooltip.default)
                        ...
                        textbutton "esc_aggressive:[persistent.jyacs_advanced_setting.get('esc_aggressive', 'None')]":
                            ...
                            hovered SetField(_tooltip, "value", _("调用agent模型对MFocus联网搜集的信息整理一�?\n启用此功能会改善模型对联网检索信息的专注能力, 但也会降低涉及联网搜索query的响应速度."))
                            unhovered SetField(_tooltip, "value", _tooltip.default)
                        textbutton "amt_aggressive: [persistent.jyacs_advanced_setting.get('amt_aggressive', 'None')]":
                            ...
                            hovered SetField(_tooltip, "value", _("要求MFocus预检MTrigger内容(若存�?, 以告知核心模型要求是否可以完�? \n启用此功能会改善MTrigger与核心模型的表现失步问题, 但也会降低涉及MTrigger对话的响应速度.\n当对话未使用MTrigger或仅有好感触发器, 此功能不会生�?"))
                            unhovered SetField(_tooltip, "value", _tooltip.default)
                    hbox:
                ...
                vbox:
                    ...
                    if renpy.config.debug:
                        ...
                        # 安全地处理未定义的变�?                        python:
                            ...
                            # 安全的事件检查函�?                            def safe_event_check():
                                try:
                                    # 安全地检查标签是否存�?                                    greeting_check = renpy.has_label('jyacs_greeting') and renpy.seen_label('jyacs_greeting')
                        ...
                        hbox:
                            # 使用安全的变量引�?                            text "未读消息: [rev_unseen_len] | [rev_unseen_str]":
                                size 15
                        hbox:
                            # 使用安全的函�?                            text "邮件状态检�? [safe_mail_check()]":
                                size 15
                        hbox:
                            # 使用安全的函�?                            text "事件状态检�? [safe_event_check()]":
                                size 15
                        hbox:
                            textbutton "输出Event信息到日�?:
                        ...
                        # 安全地处理事件推�?                        python:
                            def safe_push_event(event_name):
                                ...
                                try:
                                    ...
                                    if hasattr(store, 'jyacs_log'):
                                        store.jyacs_log("推送事件失�? {}".format(e), "ERROR")
                    ...
                    hbox:
                        text _("累计发信�? [store.jyacs.jyacs.stat.get('mpostal_count')]")
                    ...
                    hbox:
                        textbutton _("服务提供节点: [JyacsProviderManager.get_server_by_id(persistent.jyacs_setting_dict.get('provider_id')).get('name', 'Unknown')]"):
                            ...
                            hovered SetField(_tooltip, "value", _("设置服务器节�?))
                            unhovered SetField(_tooltip, "value", _tooltip.default)
                    hbox: 
                        textbutton _("自动重连: [persistent.jyacs_setting_dict.get('auto_reconnect')]"):
                            ...
                            hovered SetField(_tooltip, "value", _("连接断开时自动重�?))
                            unhovered SetField(_tooltip, "value", _tooltip.default)

                        textbutton _("严格反劫�? [persistent.jyacs_setting_dict.get('strict_mode')]"):
                            action ToggleDict(persistent.jyacs_setting_dict, "strict_mode", True, False)
                            hovered SetField(_tooltip, "value", _("严格模式�? 将会在每次发送时携带cookie信息"))
                            unhovered SetField(_tooltip, "value", _tooltip.default)
                    hbox:
                    ...
                    hbox:
                        ...
                        textbutton _("锁定最佳实�?):
                            ...
                            hovered SetField(_tooltip, "value", _("锁定seed�?2, 该设置覆盖高级参数中的seed\n启用会完全排除生成中的随机�? 在统计学上稳定性更�?))
                            unhovered SetField(_tooltip, "value", _tooltip.default)
                    ...
                    hbox:
                        textbutton _("使用存档数据: [persistent.jyacs_setting_dict.get('sf_extraction')]"):
                            ...
                            hovered SetField(_tooltip, "value", _("关闭�? 模型将不会使用存档数据\n每次重启游戏将自动上传存�?))
                            unhovered SetField(_tooltip, "value", _tooltip.default)

                    hbox:
                        textbutton _("当前使用会话: [persistent.jyacs_setting_dict.get('chat_session')]"):
                            action Function(store.change_chatsession)
                            hovered SetField(_tooltip, "value", _("chat_session�?为单轮对话模�? 不同的对话之间相互独�? 需要分别上传存�?))
                            unhovered SetField(_tooltip, "value", _tooltip.default)
                    ...
                    hbox:
                        ...
                        textbutton _("控制台字�? [persistent.jyacs_setting_dict.get('console_font')]"):
                    ...
                    hbox:
                        textbutton _("清除玩家补充信息: 当前共有[len(persistent.jyacs_player_additions)]条"):
                            ...
                            hovered SetField(_tooltip, "value", _("由你补充的一些数�? 增删后需要重新上传存�?))
                            unhovered SetField(_tooltip, "value", _tooltip.default)
                    ...
                    hbox:
                        textbutton _("MSpire: [persistent.jyacs_setting_dict.get('mspire_enable')]"):
                            ...
                            hovered SetField(_tooltip, "value", _("是否允许由MSpire生成的对�? MSpire不受MFocus影响, 需要关闭重复对�?))
                            unhovered SetField(_tooltip, "value", _tooltip.default)
                    ...
                    hbox:
                        textbutton _("MSpire 使用缓存"):
                            ...
                            hovered SetField(_tooltip, "value", _("启用MSpire缓存, 且使用默认高级参数并固定种子�?2\n"))
                            unhovered SetField(_tooltip, "value", _tooltip.default)
                    ...
                    hbox:
                        textbutton _("日志等级:[safe_get_log_level()]"):
                            ...
                            hovered SetField(_tooltip, "value", _("这将影响日志文件中每条log的等�? 低于该等级的log将不会记录\n这也会影响其他子模组"))
                            unhovered SetField(_tooltip, "value", _tooltip.default)
                        ...
                        textbutton _("控制台log等级: [logging.getLevelName(store.jyacs.jyacs.console_logger.level)]"):
                            ...
                            hovered SetField(_tooltip, "value", _("这将影响控制台中每条log的等�? 低于该等级的log将不会记�?))
                            unhovered SetField(_tooltip, "value", _tooltip.default)
                    ...
                    hbox:
                        ...
                        bar:
                            ...
                            xsize 150
                            hovered SetField(_tooltip, "value", _("回信所需要的最低时�?))
                            unhovered SetField(_tooltip, "value", _tooltip.default)
                    ...
                    hbox:
                        ...
                        textbutton _("动态的天堂树林"):
                            ...
                            hovered SetField(_tooltip, "value", _("使用动态摇曳和改良光影的天堂树�? 略微增加渲染压力. 重启生效\n如果产生显存相关错误, 删减精灵包或禁用此选项"))
                            unhovered SetField(_tooltip, "value", _tooltip.default)
            ...
            hbox:
                ...
                textbutton _("重置设置"):
                    action [
                        ...
                        Function(renpy.notify, _("JYACS: 设置已重�?)),
                        Hide("jyacs_setting")
                    ]
    ...
    python:
        def apply_seed():
            ...
            if seed < 0 or seed > 99999:
                renpy.show_screen("jyacs_message", message=_("seed范围错误, 请重新输入种�?))
                        ...
                        for card in stat[server]:
                            hbox:
                                ...
                                text "VRAM: " + str(stat[server][card]["mean_memory"]) + " / " + str(stat[server][card]["vram"]):
                                    size 10
                                text renpy.substitute(_("平均功�? ")) + str(stat[server][card]["mean_consumption"]) + "W":
                                    size 10
                        text ""
...
screen jyacs_outdated_notice():
    frame:
        ...
        vbox:
            style_prefix "jyacs"
            spacing 20
            text _("> 你当前的生成版本过旧, 可能影响正常运行, 请升级至最新生成版�?)
            textbutton _("确定") action Return()

# 添加在文件的适当位置，例如在init -1500 python块之�?init -999 python:
    # 定义清理函数，避免未定义错误
    def _jyacs_clear():
        """清理登录信息"""
...
# 样式定义
init 20 python:
    # 确保不覆盖已有样�?    if not hasattr(style, 'jyacs_frame'):
    ...
    if not hasattr(style, 'jyacs_option_title'):
...
# 子模组标签定�?init 30 python:
    # 确保所有标签都有submod_前缀，避免与主游戏冲�?    if not renpy.has_label("submod_jyacs_init"):
        renpy.add_label("submod_jyacs_init", lambda: renpy.jump("submod_jyacs_init_impl"))

# 子模组初始化标签
label submod_jyacs_init_impl:
    python:
        # 安全地初始化子模�?        success = jyacs_init()
        if not success and hasattr(store, 'jyacs_log'):
            store.jyacs_log("子模组初始化失败", "ERROR")
    return

# 子模组设置标�?label submod_jyacs_settings:
    python:
        # 安全地显示设置界�?        if hasattr(store, 'jyacs_log'):
            store.jyacs_log("显示设置界面", "INFO")
        renpy.show_screen("jyacs_settings")
    return

# 子模组高级设置标�?label submod_jyacs_advanced_settings:
    python:
        # 安全地显示高级设置界�?        if hasattr(store, 'jyacs_log'):
            store.jyacs_log("显示高级设置界面", "INFO")
        renpy.show_screen("jyacs_advanced_settings")
    return

# 子模组重置标�?label submod_jyacs_reset:
    python:
        # 安全地重置子模组
        if hasattr(store, 'jyacs_log'):
            store.jyacs_log("重置子模�?, "INFO")
        reset_session()
                        ...
                        frame:
                            ...
                            vbox:
                                ...
                                hbox:
                                    ...
                                    textbutton "开�? action ToggleDict(persistent.jyacs_setting_dict, "auto_connect") style "jyacs_button"
                                
                                hbox:
                                    style "jyacs_option_hbox"
                                    text "自动重连:" style "jyacs_label"
                                    textbutton "开�? action ToggleDict(persistent.jyacs_setting_dict, "auto_reconnect") style "jyacs_button"
                                
                                hbox:
                                    style "jyacs_option_hbox"
                                    text "启用触发�?" style "jyacs_label"
                                    textbutton "开�? action ToggleDict(persistent.jyacs_setting_dict, "enable_triggers") style "jyacs_button"
                                
                                hbox:
                                    style "jyacs_option_hbox"
                                    text "启用情感系统:" style "jyacs_label"
                                    textbutton "开�? action ToggleDict(persistent.jyacs_setting_dict, "enable_emotion") style "jyacs_button"
