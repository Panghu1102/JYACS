...
init -1650 python:
    ...
    if not hasattr(persistent, 'jyacs_chat_mode'):
        persistent.jyacs_chat_mode = False
    
    if not hasattr(persistent, 'show_free_talk_button'):
        persistent.show_free_talk_button = False
    ...
    if not hasattr(persistent, 'jyacs_setting_dict'):
        persistent.jyacs_setting_dict = {
            ...
            "enable_emotion": True, "console": True, "show_console_when_reply": False,
...
label jyacs_after_load:
    python:
        jyacs_late_init()
        renpy.show_screen("jyacs_ingame_chat_ui")
    return
