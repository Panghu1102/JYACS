...
init -800 python:
    ...
    def jyacs_apply_setting(ininit=False):
        ...
        if persistent.jyacs_setting_dict.get("mspire_interval", 30) <= 10:
            persistent.jyacs_setting_dict["mspire_interval"] = 10

        store.jyacs.jyacs.auto_reconnect = persistent.jyacs_setting_dict.get("auto_reconnect", True)
        if persistent.jyacs_setting_dict.get("use_custom_model_config", False):
            jyacs_apply_advanced_setting()
        else:
            store.jyacs.jyacs.modelconfig = {}

        if persistent.jyacs_setting_dict.get("42seed", False):
        ...
        store.jyacs.jyacs.sf_extraction = persistent.jyacs_setting_dict.get("sf_extraction", False)
        store.jyacs.jyacs.chat_session = persistent.jyacs_setting_dict.get("chat_session", 0)
        store.jyacs.jyacs.model = persistent.jyacs_setting_dict.get("jyacs_model", "jyacs_main")
        store.jyacs.jyacs.mspire_use_cache = persistent.jyacs_setting_dict.get("mspire_use_cache", True)

        # 设置控制台字体（安全地检查对象是否存在）
        if hasattr(store, 'jyacs_console') and hasattr(store.jyacs_console, 'font'):
            store.jyacs_console.font = persistent.jyacs_setting_dict.get("console_font", "mod_assets/font/SarasaMonoTC-SemiBold.ttf")

        store.jyacs.jyacs.target_lang = persistent.jyacs_setting_dict.get("target_lang", "zh_cn")
        store.jyacs.jyacs.mspire_category = persistent.jyacs_setting_dict.get("mspire_category", "")
        store.jyacs.jyacs.mspire_type = persistent.jyacs_setting_dict.get("mspire_search_type", "")

        # 设置日志级别（安全地检查对象是否存在）
        if hasattr(store, 'jyacs_submod_utils') and hasattr(store.jyacs_submod_utils, 'submod_log'):
            store.jyacs_submod_utils.submod_log.level = persistent.jyacs_setting_dict.get("log_level", "INFO")
        if hasattr(store.jyacs.jyacs, 'console_logger'):
            store.jyacs.jyacs.console_logger.level = persistent.jyacs_setting_dict.get("log_conlevel", "INFO")

        store.jyacs.jyacs.mspire_session = 0
        store.jyacs.jyacs.provider_id = persistent.jyacs_setting_dict.get("provider_id", 1)
        store.jyacs.jyacs.max_history_token = persistent.jyacs_setting_dict.get("max_history_token", 4096)
        store.jyacs.jyacs.enable_strict_mode = persistent.jyacs_setting_dict.get("strict_mode", False)
    ...
    def change_chatsession():
        """切换聊天会话"""
        current_session = persistent.jyacs_setting_dict.get("chat_session", 0)
        current_session += 1
        if current_session not in range(0, 10):
            current_session = 0
        persistent.jyacs_setting_dict["chat_session"] = current_session
    ...
    def change_loglevel():
        ...
        curr = l.index(persistent.jyacs_setting_dict.get("log_level", logging.INFO))
    ...
    def change_conloglevel():
        ...
        import logging
        l = [logging.NOTSET, logging.DEBUG, logging.INFO, logging.WARNING, logging.ERROR, logging.CRITICAL]
        curr = l.index(persistent.jyacs_setting_dict.get("log_conlevel", logging.INFO))
