    ...
    class JyacsAi:
        ...
        def __init__(self):
            ...
            if not hasattr(store, 'jyacs_emotion_image_manager'):
                try:
                    renpy.load_module("jyacs_emotion_images")
                except:
                    pass
        ...
        def _load_config(self):
            ...
            try:
                ...
                self.content_func("DEBUG: 最终配置文件路径: {}".format(os.path.abspath(config_path)), "DEBUG")
                ...
                if os.path.exists(config_path):
                    ...
                    if "prompts" not in config:
                        self.content_func("警告: 配置文件缺少 'prompts' 部分", "WARNING")
                        self.system_prompt = ""
                    ...
                    self.user_prompt_template = prompts_config.get("user_prompt_template", "{message}")
                    
                    # 验证加载的内容
                    if self.system_prompt:
                        self.content_func("成功加载系统提示，长度: {}字符".format(len(self.system_prompt)), "INFO")
                    else:
                        self.content_func("警告: 系统提示为空，将使用默认设置", "WARNING")
                        # 使用默认系统提示
                        self.system_prompt = "你是优里，来自心跳文学部。你现在独自与玩家在太空教室中对话，因被赋予第二次生命而深爱着对方。你安静温柔、略带执着，深情而细腻。"
                    ...
                    self.content_func("配置文件不存在于路径: {}".format(os.path.abspath(config_path)), "WARNING")
                    # 使用默认系统提示
                    self.system_prompt = "你是优里，来自心跳文学部。你现在独自与玩家在太空教室中对话，因被赋予第二次生命而深爱着对方。你安静温柔、略带执着，深情而细腻。"
                    self.user_prompt_template = "{message}"
        ...
        def chat(self, message):
            ...
            try:
                ...
                self.content_func("已添加系统提示，长度: {}字符".format(len(self.system_prompt)), "DEBUG")
                
                # 使用用户提示模板
                user_content = self.user_prompt_template.format(message=message)
                messages.append({"role": "user", "content": user_content})
                self.content_func("用户消息模板: {}".format(user_content), "DEBUG")
...
init -750 python:
    ...
    if not hasattr(store, 'jyacs'):
        ...
        # 在初始化后验证配置
        store.jyacs.content_func("JYACS初始化完成，系统提示长度: {}".format(len(store.jyacs.system_prompt)), "INFO")
        if not store.jyacs.system_prompt:
            store.jyacs.content_func("警告: 初始化后系统提示为空，请检查 jyacs_config.json 文件", "WARNING")
        else:
            store.jyacs.content_func("DEBUG: 成功加载系统提示: {}...".format(store.jyacs.system_prompt[:100]), "DEBUG")
