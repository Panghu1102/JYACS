...
# 应用设置并记录事件统计
init -1000 python:
    """集中初始化：保证引用函数存在并注册到 store，避免 NameError。"""

    # ---------- 安全获取或占位 ---------- #
    def _ensure_fn(name, default):
        """确保全局存在指定函数，否则使用默认实现占位"""
        if name not in globals() or not callable(globals()[name]):
            globals()[name] = default
        return globals()[name]

    # 占位实现们 ---------------------------------------------------------
    _ensure_fn("jyacs_apply_setting", lambda *a, **k: None)
    _ensure_fn("log_eventstat", lambda *a, **k: None)

    def _default_splitter():
        return store.TalkSplitV2() if hasattr(store, "TalkSplitV2") else None
    _ensure_fn("create_sentence_splitter", _default_splitter)

    _ensure_fn(
        "process_user_input",
        lambda text: store.key_replace(text) if hasattr(store, "key_replace") else text,
    )

    def _default_ai_splitter(reply_text):
        if not hasattr(store, "TalkSplitV2"):
            return [reply_text]
        s = store.TalkSplitV2(); out = []
        for ch in reply_text:
            s.add_part(ch)
            seg = s.split_present_sentence()
            if seg:
                out.append(seg)
        out.extend(s.announce_stop())
        return out
    _ensure_fn("process_ai_reply_with_splitter", _default_ai_splitter)

    _ensure_fn("process_user_message", lambda msg: globals()["process_user_input"](msg))
    ...
    try:
        ...
        if hasattr(store, "jyacs_log"):
            store.jyacs_log("jyacs_apply_setting error: {}".format(e), "ERROR")

    try:
        log_eventstat()
    except Exception as e:
        if hasattr(store, "jyacs_log"):
            store.jyacs_log("log_eventstat error: {}".format(e), "ERROR")
    ...
    mapping = {
        ...
        "jyacs_apply_advanced_setting": globals().get("jyacs_apply_advanced_setting"),
        "reset_session": globals().get("reset_session"),
        "output_chat_history": globals().get("output_chat_history"),
        "upload_chat_history": globals().get("upload_chat_history"),
        "change_chatsession": globals().get("change_chatsession"),
        "reset_player_information": globals().get("reset_player_information"),
        "export_player_information": globals().get("export_player_information"),
        "update_model_setting": globals().get("update_model_setting"),
        "change_loglevel": globals().get("change_loglevel"),
        "change_conloglevel": globals().get("change_conloglevel"),
        "log_eventstat": log_eventstat,
    }
    for _k, _v in mapping.items():
        if _v and not hasattr(store, _k):
            setattr(store, _k, _v)
...
# 子模组标签定义
init 30 python:
    # 确保所有标签都有submod_前缀，避免与主游戏冲突
    if not renpy.has_label("submod_jyacs_init"):
