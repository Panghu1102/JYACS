...
label submod_jyacs_chat_start:
    ...
    jump submod_jyacs_chat_loop

label submod_jyacs_chat_loop:
    python:
        # 检查JYACS状态
            ...
            if hasattr(store.jyacs, 'is_ready_to_input'):
                if not store.jyacs.is_ready_to_input():
                    if hasattr(store.jyacs, 'is_failed') and store.jyacs.is_failed():
                        ...
                        renpy.jump("submod_jyacs_chat_start")
                    else:
                        renpy.notify("JYACS正在连接中，请稍候...")
                        renpy.pause(1.0)
                        renpy.jump("submod_jyacs_chat_loop")
...
# 聊天界面屏幕
init 15 screen jyacs_chat_screen():
    style_prefix "jyacs"
...
# 控制台界面
init 15 screen jyacs_console_teaching():
    zorder 50
...
# 输入界面
init 15 screen jyacs_input_screen(prompt):
    style_prefix "input"
...
# 控制台显示/隐藏 - 添加submod_前缀
label submod_jyacs_show_console:
    if persistent.jyacs_setting_dict['console']:
        show screen jyacs_console_teaching
        show yuri at t22
    return "normal"

label submod_jyacs_hide_console:
    if persistent.jyacs_setting_dict['console']:
        hide screen jyacs_console_teaching
        show yuri at t11
    return "normal"
